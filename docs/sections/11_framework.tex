...
En esta sección se describe el problema de las batallas del formato \emph{gen9randombattle} como POMDPs (\textit{Partially Observable Markov Decision Process}), o Proceso de decisión de Markov parcialmente observable. La tabla \ref{tab:caracteristicas} contiene la nomenclatura que se utilizará en el resto de esta sección. Si se desea ver una modelización más completa y detallada, se puede consultar el anexo \ref{anexo:modeling}.
...
Debido a que el juego es parcialmente observable, el agente actúa a partir de historiales en lugar del estado completo. El historial de información del jugador $i$ hasta el turno $t$ es
$$
...
\end{equation}
donde $\sigma_1$ y $\sigma_2$ son acciones mixtas locales condicionadas al estado de información actual. En la práctica, resolver exactamente la Ec.~\eqref{eq:bellman_isaacs} es inviable para juegos realistas, pero proporciona el objeto formal correcto.
