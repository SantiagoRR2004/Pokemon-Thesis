...
\end{equation}
where $N_i$ is the team size, $X_{i,k}^t$ is the detailed state of the $k$th unit, and $A_i^t$ indicates which unit or units are currently active. In singles, $A_i^t$ is a single active index; in doubles, it is an ordered or unordered active pair depending on the implementation.
...
For singles, a typical legal action set takes the schematic form
\begin{equation}
\cA_i(s_t) \subseteq \cA_i^{\mathrm{move}}(s_t) \cup \cA_i^{\mathrm{switch}}(s_t) \cup \cA_i^{\mathrm{special}}(s_t),
\end{equation}
where move actions correspond to selecting one legal move, switch actions correspond to switching to a legal reserve unit, and special actions encode format-dependent mechanics if present.

In doubles, an action may include move choice together with target choice for each active slot, or a combination of move and switch decisions, subject to engine legality constraints.
...
u \in \cV,
\end{equation}
for example a discrete attack style, hidden move profile, or hidden item class. Each player has a team of size $N$.
...
\subsection{Singles battle version}

For clarity, consider first a singles version with one active unit per player. At turn $t$, the active indices are
\begin{equation}
a_t^{\mathrm{act},1} \in \{1,\dots,N\},
\qquad
a_t^{\mathrm{act},2} \in \{1,\dots,N\}.
\end{equation}
...
\begin{itemize}
\item attack with move type $m \in \{1,\dots,M\}$ available to the active unit;
\item switch to one legal reserve unit;
\item optionally use a small format-specific special action if included in the model.
\end{itemize}
Thus the reduced legal action set is finite and state dependent:
\begin{equation}
\cA_i^{\mathrm{mini}}(s_t^{\mathrm{mini}})
\subseteq
\{\text{moves}\}\cup\{\text{switches}\}\cup\{\text{special}\}.
\end{equation}
